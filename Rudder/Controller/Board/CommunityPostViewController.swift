...
extension CommunityPostViewController {
    ...
    func requestAddLike(){
        RequestAddLike.uploadInfo(postId: self.postId, completion: {
            likeCount in
            if likeCount == -1 {
                print("addLike error")
            }
...
extension CommunityPostViewController {
    @IBAction func touchUpAddCommentButton(_ sender: UIButton){
        ...
        RequestAddComment.uploadInfo(commentBody: commentBody, groupNum: groupNum, postId: self.postId, status: parentOrChild, completion: {
            status in
            DispatchQueue.main.async {
