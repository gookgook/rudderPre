...
class TmpViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var banInfinite: Bool = true //이거를 왜 애초에 true 했는지에 대한 근본적인 의문이 있음
    ...
    @IBAction func touchUpNotiButton(_ sender: UIButton){
        print("Noti touched")
        DispatchQueue.main.async {self.performSegue(withIdentifier: "GoNotification", sender: sender)}
        //Alert.showAlert(title: "Please wait for our next update!", message: nil, viewController: self)
    }
...
extension TmpViewController {
    @objc private func requestPosts(endPostId: Int) {
        ...
        Request.posts(categoryId: currentCategoryId,endPostId: endPostId, isMyPost: false, searchbody: "") { (posts: [Post]?) in
...
extension TmpViewController{
    @objc func touchUpLikeButton(_ sender: UIButton) {
        ...
        if TmpViewController.posts[sender.tag].isLiked == true {
            ...
            requestAddLike(cell: cell,cellTag:sender.tag)
    ...
    func requestAddLike(cell: CommunityBoardCell,cellTag: Int){
        RequestAddLike.uploadInfo(postId: cell.tag, completion: {
            likeCount in
            if likeCount == -1 {
                print("addLike error")
            }
