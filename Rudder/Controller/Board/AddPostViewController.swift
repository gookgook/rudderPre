...
extension AddPostViewController{
    @objc func touchUpPostButton(_ sender: UIButton){
        ...
        RequestAddPost.uploadInfo( categoryId: currentCategoryId, postBody: postBody, isEmageExist: true, completion: {
            postId in
            ...
            RequestPhotoUrl.uploadInfo(postId: postId, contentType: self.photoType, fileName: "mockFileName", completion: {
                url in
                guard url != nil else {
                ...
                RequestPhotoUpload.uploadInfo(photoURL: url!, photoData: self.goImage.jpegData(compressionQuality: 0)!, contentType: self.photoType,completion: {
                    status in
                    print(status)
                    guard status != -1 else {
                    ...
                    RequestPhotoComplete.uploadInfo(postId: postId, completion: {
                        status in
                        if status == 1 {print("photo success to server complete")}
                        else {print("photo success to server error")}
                    })
...
extension AddPostViewController {
    @objc private func requestCategory() {
        ...
        RequestCategory.categories(categoryTypes: nil, isUserSelectCategory: true) { (categories: [Category]?) in
