...
extension MyPostCommentViewController {
    @objc override func requestPosts(endPostId: Int, searchbody: String) { //override대매 파라미터 나둠. 안씀
        ...
        if myPostOrComment == 0 {
            Request.posts(categoryId: nil, endPostId: endPostId, isMyPost: true, searchbody: "" ) { (posts: [Post]?) in
                if let posts = posts {
                    if posts.count == 0 {
            ...
            RequestMyComment.posts(endPostId: endPostId) { (posts: [Post]?) in
                if let posts = posts {
                    if posts.count == 0 {
                        Alert.showAlert(title: "No more posts", message: nil, viewController: self)
                        self.banInfinite = true
